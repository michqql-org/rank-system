...
public class NameColourEditorGui extends Gui {
    ...
    public NameColourEditorGui(GuiHandler guiHandler, Player player, PlayerManager playerManager, Rank rank) {
        super(guiHandler, player);
        this.playerManager = playerManager;
        if(rank.getNameColour() == null)
            rank.setNameColour(new Pair<>(ChatColor.RESET, new HashSet<>()));
        this.colours = rank.getNameColour();
        build("&2Editing &f" + rank.getRankId() + "'s &2name colour", 6);
    }
    ...
    private String createPreviewLine() {
        ...
        String chat = pr != null ? pr.getChatColour() : "&f";
        String message = PREVIEW_MESSAGES[randomMessageIndex];

        return prefix + " &f" + setToString() +  player.getName() + "&f " + suffix + " " + chat + message;
    }
